...
@section('body-content')
	...
	<header class="panel-heading">
	    Direct Referral Comission Reports
	...
	@foreach( $drcReports as $drcReport)
		<tr>
	    <td>{{ $i++ }}</td>
	    <td>{{ FindUserName::userName($drcReport->user_id) }}</td>
	    <td>{{ FindUserName::userName($drcReport->related_id) }}</td>
	    <td>{{ $drcReport->amount }}</td>
	    <td>
	    	{{ date('Y-m-d', strtotime($drcReport->date)) }}
	    </td>
	</tr>
