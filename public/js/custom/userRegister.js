$(document).ready(function(){
	...
	$('#phone_number').blur(function(){
		...
		// alert(phone_number.length);exit();
		...
		else if( phone_number.length < 11 || phone_number.length > 16  )

		{

			

			$('#errMsgUserPhone2').html('Phone number should be 11 to 16 charcters');

			$('#errMsgUserPhone2').css('display','inline').fadeOut(5000);
	    	...
	    	$('#username').blur(function(){
	    		...
	    		$.ajax({
	    			...
	    			if(data === "exist"){
	    				...
	    				$('#username').focus();
						$('#errMsgUser').empty();
	    				$('#errMsgUser').html("This user name already exist");
